...
| [#9](https://github.com/Sonar-Gitar-Demos/gitar-demo-python/pull/9) | **CI Failure → Auto-Approve** | Identical diff to #4. Kept separate so the full chain can be demoed without consuming #4. | Open the PR, watch CI fail, apply Gitar's suggested fix, watch CI re-run and pass, then watch Gitar auto-approve the now-clean PR. |
